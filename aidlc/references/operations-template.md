...
Generated by AIDLC v2.5.0. Local to this workspace, do not commit.
...
last_analyzed: ""             # ISO timestamp of last project analysis (e.g. 2026-04-28T12:00:00Z)
...
version_log_dir: "versions/"  # directory for version progress files
version_log_committed: false  # set true to commit versions/ to git (useful for teams/open source)
commit_convention: ""         # e.g. conventional, semver-prefix, free-form
sensitive_patterns: []        # project-specific patterns to add to secrets scan
                              # built-in defaults already cover: token=, api_key=, password=, secret=, Bearer
                              # add project-specific ones here, e.g.: AWS_SECRET_ACCESS_KEY=, STRIPE_KEY=
```
...
> When no task is active, all fields are empty and status is "idle".
...
7. **Drift detection** — compare this file against actual project state (see below)
...
After reading this file, check for staleness:
...
- Has `last_analyzed` exceeded 30 days?
...
> After approved update, refresh `last_analyzed` timestamp.
...
7. **Create version log**: write `<version_log_dir>/v<VERSION>.md` with plan and initial progress
8. **Update Part 2** of `OPERATIONS.md` with the new version info, status `in-progress`
...
# 4. Secrets scan (built-in defaults + project-specific patterns)
git diff --cached | grep -iE 'token=.{8,}|api[_.]?key=.{8,}|password=|secret=|Bearer .{20,}<project_patterns_appended_with_pipe>' && echo "❌ SECRETS" || echo "✅ Clean"

# 5. No unintended changes
git diff --stat
```

> Built-in patterns cover common cases. Append project-specific patterns from `sensitive_patterns` in Part 1.
> Agent MUST substitute all placeholders — the pre-commit script must be copy-paste executable.
...
> If `version_log_committed: true`, also run `git add versions/v<VERSION>.md && git commit --amend --no-edit` before push.

✏️ The version is now closed. Next task starts fresh from Phase 0.
````

> **Key rule**: In Phase 6, the agent MUST substitute all placeholders with actual values from Part 1. The pre-commit script must be copy-paste executable — no placeholders left.
